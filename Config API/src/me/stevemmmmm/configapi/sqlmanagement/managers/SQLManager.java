...
public class SQLManager {
    ...
    public static ResultSet sendQuery(MySQLDatabase database, String query) {
        try {
            Connection connection = DriverManager.getConnection(database.getConnectionUrl(), database.getUsername(),
    ...
    public static <K, V> HashMap<K, V> getMap(MySQLDatabase database, String table, Class<K> keyType, String keyColumn,
            Class<V> valueType, String valueColumn) {
        try {
            Connection connection = DriverManager.getConnection(database.getConnectionUrl(), database.getUsername(),
                    database.getPassword());
            Statement statement = connection.createStatement();
            ...
            while (resultSet.next()) {
                ...
                result.put(keyType.isPrimitive() ? (K) finalKey : keyType.cast(finalKey),
                        valueType.isPrimitive() ? (V) finalValue : valueType.cast(finalValue));
            }
    ...
    @SuppressWarnings("unchecked")
    public static <T> HashMap<UUID, T> getPlayerDataMap(MySQLDatabase database, String table, String uuidColumn,
            Class<T> valueType, String valueColumn, CastInstructions valueInstructions) {
        try {
            Connection connection = DriverManager.getConnection(database.getConnectionUrl(), database.getUsername(),
                    database.getPassword());
            Statement statement = connection.createStatement();
            ...
            while (resultSet.next()) {
                ...
                if (valueInstructions != null)
                    finalValue = valueInstructions.cast(keyEntry, valueEntry);

                result.put(UUID.fromString(keyEntry),
                        valueType.isPrimitive() ? (T) finalValue : valueType.cast(finalValue));
            }
    ...
    @SuppressWarnings("unchecked")
    public static <K, V> HashMap<K, V> getComplexMap(MySQLDatabase database, String table, Class<K> keyType,
            String uuidColumn, Class<V> valueType, String valueColumn, CastInstructions keyInstructions,
            CastInstructions valueInstructions) {
        try {
            Connection connection = DriverManager.getConnection(database.getConnectionUrl(), database.getUsername(),
                    database.getPassword());
            Statement statement = connection.createStatement();
            ...
            while (resultSet.next()) {
                ...
                if (keyInstructions != null)
                    finalKey = keyInstructions.cast(keyEntry, valueEntry);
                if (valueInstructions != null)
                    finalValue = valueInstructions.cast(keyEntry, valueEntry);
    ...
    /**
     * Reads the <code>ResultSet</code> and casts the result to the specified class
     * from the given query.
     *
     * @param database   The database to connect to (PebbleHost prefix)
     * @param query      The query to be executed
     * @param resultType The class that the data from the ResultSet will be casted
     *                   to
